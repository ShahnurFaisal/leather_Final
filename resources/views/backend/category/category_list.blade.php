                                        ...
                                        <tbody class="table-border-bottom-0">
                                            @foreach ($data['category_list'] as $key => $single_category)
                                                <tr>
                                                    ...
                                                    <td>{{ $single_category->priority }}</td>
                                                    <td>{{ date('d M y',strtotime($single_category->created_at)) }}</td>
                                                    <td>
                                                        <a href="{{ route('admin.category.edit',$single_category->id) }}" class="btn btn-primary text-white">
                                                            <i class="bx bx-edit-alt me-1"></i> Edit</a>
                                                        <a class="btn btn-primary text-white" id="delete"
                                                            data-id="{{ $single_category->id }}"><i
